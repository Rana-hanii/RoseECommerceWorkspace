...
_adaptorService = inject(AuthAPIResService );
  ...
  LogOut():Observable<ILogOutRes>  {
    return this._httpClient.get<ILogOutRes>(AuthEndPoint.LOGOUT, {}).pipe(
  ...
  VerifyCode(data: IVerifyReq): Observable<IVerifyRes> {
    return this._httpClient.post<IVerifyRes>(AuthEndPoint.VERIFY, data).pipe(
  ...
  ResetPassword(data: IResetReq): Observable<IResetRes> {
    return this._httpClient.put<IResetRes>(AuthEndPoint.RESETPASSWORD, data).pipe(
